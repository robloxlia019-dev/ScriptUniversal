...
-- ══════════════════════════════════════
-- SILENT AIM - Hook no ShootGun remote
-- ══════════════════════════════════════
local originalShootGun
if ShootGunRemote then
    -- Hook FireServer para desviar a direção do tiro
    local mt = getrawmetatable(game)
    local oldNamecall
    local isProtected = pcall(function()
    ...
    if isProtected then
        ...
        mt.__namecall = newcclosure(function(self, ...)
            ...
            if method == "FireServer" and self == ShootGunRemote then
                if State.SilentAim then
                    ...
                    if chance <= State.SilentAimChance then
                        ...
                        if targetPos then
                            local args = {...}
                            -- Substitui a posição de destino pelo alvo
                            -- O jogo envia: ShootGun:FireServer(targetPosition, ...)
                            args[1] = targetPos
...
-- ══════════════════════════════════════
-- AUTO SHOT - Dispara automaticamente no alvo
-- ══════════════════════════════════════
...
RunService.Heartbeat:Connect(function()
    ...
    -- Verifica se tem arma equipada (GunTool)
    local tool = char:FindFirstChildOfClass("Tool")
    if not tool then return end
    local hasTag = false
    pcall(function()
        local CS = game:GetService("CollectionService")
        hasTag = CS:HasTag(tool, "GunTool")
    end)
    ...
    pcall(function()
        if ShootGunRemote then
            ShootGunRemote:FireServer(targetPart.Position)
        end
    end)
end)
...
-- ══════════════════════════════════════
-- ANTI-KICK (bloqueia RemoteEvent Ban)
-- ══════════════════════════════════════
local antiKickConn
local function enableAntiKick(enabled)
    if enabled then
        ...
        pcall(function()
            ...
            antiKickConn = mt.__namecall
            mt.__namecall = newcclosure(function(self, ...)
                local method = getnamecallmethod()
                if method == "FireServer" then
                    local remote = self
                    if Remotes then
                        local banRemote = Remotes:FindFirstChild("Ban")
                        if banRemote and remote == banRemote then
                            return -- bloqueia
                        end
                    end
                end
...
-- Aplica skin na arma equipada localmente (visual apenas)
...
local function applyKnifeSkin(skinName)
    local char = LocalPlayer.Character
    if not char then return end
    -- Busca pela faca no personagem ou backpack
    local function findKnife(parent)
        for _, tool in ipairs(parent:GetChildren()) do
            if tool:IsA("Tool") then
                local CS = game:GetService("CollectionService")
                local ok2, hasTag = pcall(function()
                    return CS:HasTag(tool, "KnifeTool")
                end)
                if ok2 and hasTag then return tool end
            end
        end
        return nil
    end
    local knife = findKnife(char) or findKnife(LocalPlayer:FindFirstChild("Backpack") or {})
...
WeaponTab:CreateButton({
    ...
    Callback = function()
        ...
        Rayfield:Notify({
            Title    = "Skin Aplicada",
            Content  = "Skin de arma aplicada: " .. State.SelectedGunSkin,
            Duration = 3,
        })
    end,
})
...
WeaponTab:CreateButton({
    ...
    Callback = function()
        ...
        Rayfield:Notify({
            Title    = "Skin Aplicada",
            Content  = "Skin de faca aplicada: " .. State.SelectedKnife,
            Duration = 3,
        })
    end,
})
...
if OnPlayerKilledRemote then
    OnPlayerKilledRemote.OnClientEvent:Connect(function(killedPlayer, killerPlayer)
        ...
        Rayfield:Notify({
            Title    = "Kill Effect",
            Content  = "Efeito aplicado: " .. State.SelectedKillEffect,
            Duration = 2,
        })
    end)
end
...
task.delay(2, function()
    Rayfield:Notify({
        Title    = "MVS DUELS Script",
        Content  = "Script carregado! Clique no botão 'MVS' para abrir/fechar a GUI. O botão é arrastável.",
        Duration = 6,
        Image    = 4483362458,
    })
end)
