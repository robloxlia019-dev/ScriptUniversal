...
-- ══════════════════════════════════════
-- SILENT AIM - Via CFrame Camera redirect
-- ══════════════════════════════════════
-- Abordagem sem hook de metatable: redireciona a câmera momentaneamente
-- para que o tiro natural do jogo vá para o alvo
local silentAimActive = false

local function applySilentAimCFrame()
    ...
    -- Guarda a CFrame original
    local originalCF = Camera.CFrame
    -- Aponta câmera pro alvo por 1 frame
    Camera.CFrame = CFrame.lookAt(Camera.CFrame.Position, part.Position)
    -- Restaura no próximo frame
    task.defer(function()
        pcall(function() Camera.CFrame = originalCF end)
    end)
end

-- Conecta ao clique do mouse para acionar o silent aim no momento do tiro
UserInputService.InputBegan:Connect(function(input, gpe)
    if gpe then return end
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        applySilentAimCFrame()
    end
end)

-- ══════════════════════════════════════
-- AUTO SHOT - Simula clique do mouse no alvo
-- ══════════════════════════════════════
...
RunService.Heartbeat:Connect(function()
    ...
    -- Aponta câmera para o alvo e simula clique (mais seguro que FireServer direto)
    local originalCF = Camera.CFrame
    Camera.CFrame = CFrame.lookAt(Camera.CFrame.Position, targetPart.Position)
    -- Simula o clique via VirtualInputManager se disponível
    pcall(function()
...
task.delay(2, function()
    Rayfield:Notify({
        Title    = "MVS DUELS Script v2",
        Content  = "Script carregado! [Anti-detecção BAC_9 ativo] Clique no botão 'MVS' para abrir/fechar a GUI.",
        Duration = 6,
        Image    = 4483362458,
    })
end)
